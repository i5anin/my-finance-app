...
export async function updateFinanceEntry(id, entry) {
  try {
    ...
    console.error("Ошибка при обновлении записи:", error.response ? error.response.status : error);
    throw error;
  }
}
...
export async function deleteFinanceEntry(id) {
  try {
    ...
    console.error("Ошибка при удалении записи:", error.response ? error.response.status : error);
    throw error;
  }
}
